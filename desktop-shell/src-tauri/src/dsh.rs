...
///
/// `NPM_CONFIG_PREFIX` / `NPM_CONFIG_CACHE` are deliberately **not** set: npm
/// keeps its native meaning inside the environment. `<env-root>/dsh-home/AGENTS.md`
/// (generated by dpx) explains the layout and tells an agent to pass an explicit
/// `--prefix` / `--cache` — or to use `dpx npm install` — when it means this
/// environment. `PATH` still starts with the environment's npm prefix so
/// binaries that were explicitly installed there resolve first.
...
mod tests {
    ...
    #[test]
    fn runtime_environment_does_not_hijack_npm_defaults() {
        let env = runtime_env(Path::new(r"C:\environments\test"));
